...
describe('AlexaSmartHomeV3 - Discovery', function () {
    ...
    describe('Handling', async function () {
        ...
        it('Discovery of a thermostat', async function () {
            ...
            assert.equal(response.event.payload.endpoints[0].capabilities[1].interface, 'Alexa.ThermostatController');
            assert.equal(response.event.payload.endpoints[0].capabilities[1].version, '3.2');
            assert.equal(response.event.payload.endpoints[0].capabilities[1].properties.supported.length, 2);
            assert.equal(
                response.event.payload.endpoints[0].capabilities[1].properties.supported[0].name,
                'targetSetpoint',
            );
            assert.equal(
                response.event.payload.endpoints[0].capabilities[1].properties.supported[1].name,
                'thermostatMode',
            );
            assert.equal(response.event.payload.endpoints[0].capabilities[1].configuration.supportedModes.length, 1);
            assert.equal(response.event.payload.endpoints[0].capabilities[1].configuration.supportedModes[0], 'AUTO');
        });

        it('Discovery of a thermostat with states', async function () {
            const event = await helpers.getSample('Discovery/Discovery.request.json');
            ...
            assert.equal(response.event.payload.endpoints[0].capabilities.length, 4);
            assert.equal(response.event.payload.endpoints[0].capabilities[0].type, 'AlexaInterface');
            assert.equal(response.event.payload.endpoints[0].capabilities[0].interface, 'Alexa');
            ...
            assert.equal(response.event.payload.endpoints[0].capabilities[1].configuration.supportedModes.length, 3);
            assert.equal(response.event.payload.endpoints[0].capabilities[1].configuration.supportedModes[0], 'AUTO');
            assert.equal(response.event.payload.endpoints[0].capabilities[1].configuration.supportedModes[2], 'ECO');
        });
        ...
        it('Discovery of a slider', async function () {
            const event = await helpers.getSample('Discovery/Discovery.request.json');
            ...
            assert.equal(response.event.payload.endpoints[0].endpointId, endpointId, 'Endpoint Id!');
            assert.equal(response.event.payload.endpoints[0].friendlyName, friendlyName, 'Friendly Name!');
            assert.equal(response.event.payload.endpoints[0].displayCategories.includes('OTHER'), true);
            assert.equal(response.event.payload.endpoints[0].displayCategories.length, 1);

            assert.equal(response.event.payload.endpoints[0].capabilities.length, 2);
            assert.equal(response.event.payload.endpoints[0].capabilities[0].type, 'AlexaInterface');
            assert.equal(response.event.payload.endpoints[0].capabilities[0].interface, 'Alexa');
            assert.equal(response.event.payload.endpoints[0].capabilities[1].interface, 'Alexa.RangeController');

            assert.equal(response.event.payload.endpoints[0].capabilities[1].properties.supported.length, 1);
            assert.equal(response.event.payload.endpoints[0].capabilities[1].properties.supported[0].name, 'rangeValue');
        });
    });
});
