        ...
        if (request.request.type === 'LaunchRequest') {
            ...
            if (request.request.intent) {
                if (request.request.intent.name === 'AMAZON.HelpIntent') {
                    ...
                    if (lang === 'en') {
                        callback(getResponse('This skill will pass all recognized words to your ioBroker installation and you can configure actions and responses there!', true));
                    } else if (lang === 'ru') {
                        callback(getResponse('Этот навык передаст все распознанные слова в вашу установку ioBroker, и вы сможете настроить действия и ответы там!', true));
                    } else {
                        callback(getResponse('Dieser Skill gibt alle erkannten Wörter an Deine ioBroker-Installation weiter und Du kannst dort Aktionen und Antworten konfigurieren!', true));
                    }
