...
function AlexaCustom(adapter) {
    ...
    this.process = (request, isEnabled, callback) => {
        ...
        if (!isEnabled) {
            if (lang === 'en') {
                callback(getResponse('The service is not activated. Please enable Alexa integration in the iot Adapter.', true));
            } else if (lang === 'ru') {
                callback(getResponse('Услуга не активирована. Включите интеграцию Alexa в адаптере Интернета вещей.', true));
            } else {
                callback(getResponse('Der Service ist nicht aktiviert. Bitte aktiviere die Alex-Integration im iot-Adapter um Ihn zu aktivieren.', true));
            }
            return;
        }

        const sessionId = (request.session && request.session.sessionId) ? request.session.sessionId : '';
        adapter.log.debug(`Custom Skill ${isV2Skill ? 'V2' : 'V1'} request: ${request.request.type}/${request.request.intent.name} for session ${sessionId}, dialog: ${request.request.dialogState}`);

        if (request.request.type === 'LaunchRequest') {
            adapter.log.debug(request.request.type);
            if (lang === 'en') {
                callback(getResponse('Hello, what do you want to know or control?', 'What do you want to know or control?'));
            } else if (lang === 'ru') {
                callback(getResponse('Привет. Что вы хотите знать или контролировать?', 'Что вы хотите знать или контролировать?'));
            } else {
                callback(getResponse('Hallo, was möchtest Du wissen oder steuern?', 'Was möchtest Du wissen oder steuern?'));
            }
            this.touchSession(sessionId, isV2Skill);
            ...
            if (request.request.intent) {
                ...
                if (request.request.intent.name === 'AMAZON.HelpIntent') {
                    if (lang === 'en') {
                        callback(getResponse('This skill will pass all recognized words to your ioBroker installation and you can configure actions and responses there! What do you want to know or control?', 'What do you want to know or control?'));
                    } else if (lang === 'ru') {
                        callback(getResponse('Этот навык передаст все распознанные слова в вашу установку ioBroker, и вы сможете настроить действия и ответы там! Что вы хотите знать или контролировать?', 'Что вы хотите знать или контролировать?'));
                    } else {
                        callback(getResponse('Dieser Skill gibt alle erkannten Wörter an Deine ioBroker-Installation weiter und Du kannst dort Aktionen und Antworten konfigurieren! Was möchtest Du wissen oder steuern?', 'Was möchtest Du wissen oder steuern?'));
                    }
                    ...
                    if (request.request.intent.name === 'AMAZON.YesIntent') {
                        ...
                        // Only relevant for v2 Skill, get the startwords out of the intent name
                    ...
                    if (!request.request.intent.slots && textsLog.length === 0) { // No slots and nothing from above
                        ...
                        if (lang === 'en') {
                            callback(getResponse('I did not hear you. Please repeat your question!'));
                        } else if (lang === 'ru') {
                            callback(getResponse('Я не слышал тебя. Пожалуйста, повторите свой вопрос!'));
                        } else {
                            callback(getResponse('Ich habe Dich nicht verstanden. Bitte wiederhole deine Frage!'));
                        }
                        return;
                    }
                            ...
                            if (adapter.config.text2command !== null && adapter.config.text2command !== undefined && adapter.config.text2command !== '') {
                                adapter.getForeignState(`system.adapter.text2command.${adapter.config.text2command}.alive`, (err, state) => {
                                    if (state && state.val) {
                                        ...
                                        openSessions[sessionId].callback = callback;
