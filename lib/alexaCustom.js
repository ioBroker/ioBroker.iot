...
function AlexaCustom(adapter) {
    ...
    this.setResponse = (resp, shouldEndSession) => {
        ...
        if (!session) {
            const sessions = Object.keys(openSessions).filter(
                sessionId => !!(openSessions[sessionId].responseTimer && openSessions[sessionId].callback),
            );
    ...
    this.processAfter = (request, text, shouldEndSession, callback) => {
        ...
        const isV2Skill =
            openSessions[sessionId].isV2Skill !== undefined
                ? openSessions[sessionId].isV2Skill
                ...
                if (request.request.intent.name === 'AMAZON.HelpIntent') {
                    ...
                    if (originalIntentName.startsWith('queryIntentStarteDialog')) {
                        ...
                        if (intent) {
                            ...
                            if (
                                adapter.config.text2command !== null &&
                                adapter.config.text2command !== undefined &&
                                adapter.config.text2command !== ''
                            ) {
                                const state = await adapter.getForeignStateAsync(
                                    `system.adapter.text2command.${adapter.config.text2command}.alive`,
                                );
                                if (state && state.val) {
