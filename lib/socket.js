...
// do not send too many state updates
    ...
    function socketEvents(socket) {
        if (socket.conn) {
            that.adapter.log.info(' Connected ' + socket._acl.user);
        } else {
            that.adapter.log.info('Trying to connect as ' + socket._acl.user);
        }
        ...
        if (!socket.conn) {
            ...
            socket.on('connect', function () {
                ...
                this.emit('apikey', that.settings.apikey, that.settings.version, that.settings.uuid, function (err, validTill) {
                    if (validTill) {
                        that.adapter.setState('info.remoteTill', new Date(validTill).toISOString(), true);
                    }
                    ...
                    if (!err) {
                        ...
                        if (err.indexOf('Please buy remote access to use pro.') !== -1) {
                            // stop adapter to not disturb the cloud
