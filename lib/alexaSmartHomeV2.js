function AlexaSH2(adapter) {
    ...
    function processState(states, id, room, func, alexaIds, groups, names, result) {
        ...
        if(states[id].common.role == "level.color.hue") {
            var sat = findRole(states,id,"level.color.saturation");
            var bri = findRole(states,id,"level.dimmer");
            var on = findRole(states,id,"switch");
        ...
        try {
            ...
            if (states[id].common.write === false) {
                ...
                if (type === 'number') {
                    ...
                    } else if (states[id].common.role === 'level.color.temperature') {
                     ...
                     /* //  var ids    = JSON.parse(groups[friendlyNames[n]].additionalApplianceDetails.ids);
                       ...
                       // groups[friendlyNames[n]].additionalApplianceDetails.ids   = JSON.stringify(ids);
                        ...
                        for(var i=0; i<addids.length; i++) {
                            var newid = addids[i];
                            var _parts = newid.split('.');
                            _parts.pop();
                            var channel = _parts.join('.');
                            newid = newid.substring(channel.length+1);
                            if(groups[friendlyNames[n]].additionalApplianceDetails.channels.hasOwnProperty(channel)) {
                                groups[friendlyNames[n]].additionalApplianceDetails.channels[channel].push(newid+" "+states[id].common.role);
                            } else {
                                groups[friendlyNames[n]].additionalApplianceDetails.channels[channel] = [newid+" "+states[id].common.role];
                            }
                        }
                    } else {
                        ...
                        for (var a = 0; a < names[friendlyNames[n]].actions.length; a++) {
                        ...
                        var oldobjDetails = names[friendlyNames[n]].additionalApplianceDetails;
                        ...
                        var addids = [names[friendlyNames[n]].additionalApplianceDetails.id, id];
                        for(var i=0; i<addids.length; i++) {
                            var newid = addids[i];
                            var _parts = newid.split('.');
                            _parts.pop();
                            ...
                            newid = newid.substring(channel.length+1);
                            if(groups[friendlyNames[n]].additionalApplianceDetails.channels.hasOwnProperty(channel)) {
                                groups[friendlyNames[n]].additionalApplianceDetails.channels[channel].push(newid+" "+states[addids[i]].common.role);
                            } else {
                                groups[friendlyNames[n]].additionalApplianceDetails.channels[channel] = [newid+" "+states[addids[i]].common.role];
                            }
                        }
    ...
    function controlOnOff(toggle, id, value, writeStates, callback) {
        adapter.getForeignObject(id, function (err, obj) {
            ...
            if(toggle && toggle !== id && ( !value || (!byON || byON === "-"))){
    ...
    function controlColorTemperature(temperature, id, value, writeStates, callback) {
        adapter.getForeignObject(id, function (err, obj) {
            ...
            if (temperature && temperature !== id) {
                adapter.log.debug('Won\'t control '+id+' as we have a temperature control available for this channel.');
                if (callback) callback();
                return;
            }
            adapter.log.debug('Controlling: ' + id + " setting to " + value);

            adapter.setForeignState(id, 1000000/value, function (err) {
                if (err) adapter.log.error('Cannot switch device: ' + err);
                if (callback) callback();
            });
        });
    ...
    function controlColorTemperatureDelta(temperature, id, delta, writeStates, callback) {
        adapter.getForeignObject(id, function (err, obj) {
            ...
            adapter.getForeignState(id, function (err, state) {
                ...
                adapter.setForeignState(id, value, function (err) {
                    if (err) adapter.log.error('Cannot switch device: ' + err);
                    if (callback) callback(null, 1000000/value);
                });
            });
        });
    }
    ...
    function findRoleInChannel(channel, role) {
        for(var i=0; i<channel.length; i++) {
            var split = channel[i].split(" ");
            adapter.log.debug("Split: "+ split[1] + " "+ role + " " + channel)
            if(split[1] === role)
                return split[0];
        }
        return null;
    ...
    this.process = function (request, isEnabled, callback) {
        ...
        if (request && request.payload && request.payload.appliance && request.payload.appliance.additionalApplianceDetails) {
            if(request.payload.appliance.additionalApplianceDetails.group) {
                channels = JSON.parse(request.payload.appliance.additionalApplianceDetails.channels);
            } else {
                var id = request.payload.appliance.additionalApplianceDetails.id;
                var role = request.payload.appliance.additionalApplianceDetails.role;
                var _parts = id.split('.');
                var data = _parts.pop();
                var channel = _parts.join('.');
                channels[channel] = [data+" "+role];
            }
        ...
        switch (request.header.name) {
            case 'DiscoverAppliancesRequest':
                ...
                for (var j = 0; j < smartDevicesCopy.length; j++) {
                    ...
                    if (smartDevicesCopy[j].additionalApplianceDetails.channels         !== undefined)
                        smartDevicesCopy[j].additionalApplianceDetails.channels = JSON.stringify(smartDevicesCopy[j].additionalApplianceDetails.channels);
                }
            ...
            case 'TurnOnRequest':
                ...
                for(var channel in channels) {
                    var ids = [];
                    var toggle = findRoleInChannel(channels[channel], "switch");
                    toggle = (toggle) ? channel + "." + toggle : null;
                    count = channels[channel].length;
                    for (i = 0; i < channels[channel].length; i++) {
                        var id = channel + "." + channels[channel][i].split(' ')[0];
                        
                        adapter.log.debug("Controlling: " + id);
                        controlOnOff(toggle, id, true, Object.keys(channels).length === 1, function () {
                            if (!--count) {
            ...
            case 'TurnOffRequest':
                ...
                for(var channel in channels) {
                    ...
                    for (i = 0; i < channels[channel].length; i++) {
                        ...
                        controlOnOff(toggle, id, false, Object.keys(channels).length === 1, function () {
                            if (!--count) {
            ...
            case 'SetPercentageRequest':
                ...
                for(var channel in channels) {
                    var ids = [];
                    var dimmer = findRoleInChannel(channels[channel], "level.dimmer");
                    adapter.log.debug("DATA: " + JSON.stringify(channels[channel]));
                    dimmer = (dimmer) ? channel + "." + dimmer : null;
            ...
            case 'SetColorRequest':
                ...
                for(var channel in channels) {
                    var ids = [];
                    var hue = findRoleInChannel(channels[channel], "level.color.hue");
                    if(hue) {
                        adapter.log.debug(" Using HUE for: " + channel);
                        hue = channel+"."+hue;
                        var sat = findRoleInChannel(channels[channel], "level.color.saturation");
                        var bri = findRoleInChannel(channels[channel], "level.dimmer");
                        if(sat && bri) {                            
                            sat = channel+"."+findRoleInChannel(channels[channel], "level.color.saturation");
                            bri = channel+"."+findRoleInChannel(channels[channel], "level.dimmer");
                            var color = JSON.parse(JSON.stringify(request.payload.color));
                            controlColorHue(sat, hue, bri, color, Object.keys(channels).length === 1, doneCallback);
                        }
                        continue;
                    }

                    var rgb = findRoleInChannel(channels[channel], "level.color.rgb");
                    if(rgb) {
            ...
            case 'SetColorTemperatureRequest':
                for(var channel in channels) {
                    var ids = [];
                    var temperature = findRoleInChannel(channels[channel], "level.color.temperature");
                    temperature = (temperature) ? channel + "." + temperature : null;
                    count = channels[channel].length;
                    for (i = 0; i < channels[channel].length; i++) {
                        var id = channel + "." + channels[channel][i].split(' ')[0];
                ...
                writeResponse(request.payload.appliance.applianceId, 'K', request.payload.colorTemperature.value);
                break;
            ...
            case 'IncrementColorTemperatureRequest':
                for(var channel in channels) {
                    ...
                    for (i = 0; i < channels[channel].length; i++) {
                        var id = channel + "." + channels[channel][i].split(' ')[0];
                        adapter.log.debug("Controlling temperature kelvin: " + id);
                        controlColorTemperatureDelta(temperature, id, -50, Object.keys(channels).length === 1, function (err, res) {
                            if (!--count) {
