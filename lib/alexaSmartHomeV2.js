function AlexaSH2(adapter) {
            ...
            if (states[id].common.write === false) {
                ...
                if (type === 'number') {
                    if (states[id].common.unit === 'C' || states[id].common.unit === 'C°' || states[id].common.unit === '°C' ||
                        ...
                        type = '';
                    } else if (states[id].common.role === 'level.color.hue') {
    ...
    function controlColorRgb(id, color, writeStates, callback) {
        ...
        adapter.getForeignObject(id, function (err, obj) {
            ...
            var rgb = hsvToRgb(color.hue/360, color.saturation, color.brightness);
    ...
    function controlColorTemperatureDelta(temperature, id, delta, writeStates, callback) {
        adapter.getForeignObject(id, function (err, obj) {
            ...
            adapter.log.debug(`objUnit = ${unit}, friendlyUnit = ${friendlyUnit}`);
            ...
            adapter.getForeignState(id, function (err, state) {
                ...
                adapter.log.debug(`min = ${minValue}, max = ${maxValue}, value = ${value}, offset = ${delta * deltaFactor * (maxValue - minValue)}`);
                value = Math.round(value + delta * deltaFactor * (maxValue - minValue));
                adapter.log.debug(`${value}`);
                value = Math.max(minValue, Math.min(value, maxValue));
                adapter.log.debug(`clamped: ${value}`);
    ...
    this.process = function (request, isEnabled, callback) {
        ...
        if (request && request.payload && request.payload.appliance && request.payload.appliance.additionalApplianceDetails) {
            var details = null;
            for(var i=0; i<smartDevices.length; i++) {
            ...
            case 'DecrementTargetTemperatureRequest':
                ...
                for(var channel in channels) {
                    for (i = 0; i < channels[channel].length; i++) {
                        var id = channel + "." + channels[channel][i].split(' ')[0];
                        adapter.log.debug("Controlling percentage: " + id);
                        controlTemperatureDelta(ids, request.payload.deltaTemperature.value * (-1), Object.keys(channels).length === 1, function (err, response) {
                            if (!--count) {
