    ...
    function getSmartName(states, id) {
        if (!id) {
            if (!adapter.config.noCommon) {
                ...
                return states && states.common && states.common.custom && states.common.custom[adapter.namespace]
                    ? states.common.custom[adapter.namespace].smartName
                    : undefined;
            }
        } else if (!adapter.config.noCommon) {
            ...
            return states[id] &&
                ...
                ? states[id].common.custom[adapter.namespace].smartName || null
                : null;
        }
    }
    ...
    this.getDevices = function (callback) {
        ...
        adapter.getObjectView('system', 'state', {}, (err, _states) => {
            ...
            adapter.getObjectView('system', 'enum', {}, (err, doc) => {
                ...
                // Check that all applianceId are unique
                const uniqueIds = [];
                for (let i = result.length - 1; i >= 0; i--) {
                    ...
                    if (result[i].actions.length > 100) {
                        adapter.log.error(`[ALEXA] Too many capabilities by "${result[i].applianceId}": ${result[i].capabilities.length}`);
                        result.splice(i, 1);
                    }
                }
