...
function AlexaSH2(adapter) {
    ...
    function getSmartName(statesOrObj, id) {
        if (!id) {
            if (!adapter.config.noCommon) {
                return statesOrObj.common.smartName;
            }
            return statesOrObj?.common?.custom[adapter.namespace]
                ? statesOrObj.common.custom[adapter.namespace].smartName
                : undefined;
        }
        if (!adapter.config.noCommon) {
            return statesOrObj[id].common.smartName;
        }
        return statesOrObj[id]?.common?.custom?.[adapter.namespace]
            ? statesOrObj[id]?.common?.custom?.[adapter.namespace].smartName || null
            : null;
    }
    ...
    function processState(states, id, room, func, alexaIds, groups, names, result) {
        // Make sure to also add the other necessary Endpoints:
        ...
        try {
            ...
            if (states[id]) {
                ...
                if (states[id].native && !adapter.config.noCommon) {
            ...
            for (let i = friendlyNames.length - 1; i >= 0; i--) {
                ...
                if (!friendlyNames[i]) {
                    ...
                    friendlyNames[i] = friendlyNames[i]
                        .substring(0, 128)
                        .replace(/[^.a-zA-Z0-9äÄüÜöÖßÉéÈèÀàÂâÊêÙùÛûÇçÎîËëÏïŸÿÔôŒœãõìòáíóú]+/g, ' ');
                }
            }
                ...
                if (names[friendlyNames[n]]) {
                    ...
                    if (groups[friendlyNames[n]]) {
                       ...
                       // groups[friendlyNames[n]].additionalApplianceDetails.ids   = JSON.stringify(ids);
                        ...
                        actions.forEach(
                            action =>
                                !groups[friendlyNames[n]].actions.includes(action) &&
                                groups[friendlyNames[n]].actions.push(action),
                        );
                        ...
                        for (let i = 0; i < addids.length; i++) {
                            ...
                            if (groups[friendlyNames[n]].additionalApplianceDetails.channels.hasOwnProperty(channel)) {
                                groups[friendlyNames[n]].additionalApplianceDetails.channels[channel].push({
                                    id: newid,
                                    role: states[addids[i]].common.role,
                                    actions,
                                });
                            } else {
                                groups[friendlyNames[n]].additionalApplianceDetails.channels[channel] = [
                                    { id: newid, role: states[addids[i]].common.role, actions },
                                ];
                            }
                        }
                    } else {
                        ...
                        groups[friendlyNames[n]].additionalApplianceDetails.byONs[oldObjDetails.id] =
                            oldObjDetails.byON;
                        groups[friendlyNames[n]].additionalApplianceDetails.names[oldObjDetails.id] =
                            oldObjDetails.name;
                        groups[friendlyNames[n]].additionalApplianceDetails.smartTypes[oldObjDetails.id] =
                            oldObjDetails.smartType;
                        groups[friendlyNames[n]].additionalApplianceDetails.byONs[id] = type;
                        groups[friendlyNames[n]].additionalApplianceDetails.names[id] = name;
                        groups[friendlyNames[n]].additionalApplianceDetails.smartTypes[id] = smartType;
                        ...
                        for (let i = 0; i < addIDs.length; i++) {
                            ...
                            if (groups[friendlyNames[n]].additionalApplianceDetails.channels.hasOwnProperty(channel)) {
                                groups[friendlyNames[n]].additionalApplianceDetails.channels[channel].push({
                                    id: newID,
                                    role: states[addIDs[i]].common.role,
                                    actions: newID === id ? actions : oldActions,
                                });
                            } else {
                                groups[friendlyNames[n]].additionalApplianceDetails.channels[channel] = [
                                    {
    ...
    async function controlPercent(dimmer, id, value, writeStates) {
        ...
        if (obj.common.type === 'boolean') {
            ...
            adapter.config.deviceOffLevel &&
            value >= adapter.config.deviceOffLevel &&
            (!obj.common.role || !obj.common.role.includes('blind'))
        ) {
            valuesON[id] = value;
    ...
    async function controlLock(id, writeStates) {
        ...
        if (obj.native.LOCK_VALUE === undefined && obj.native.CONTROL_VALUE === undefined) {
            adapter.log.warn(
                `[ALEXA] Cannot choose value for lock: Please define in "${id}" the "native.LOCK_VALUE" or "native.CONTROL_VALUE"  with locking value`,
            );
    ...
    async function getLock(id, writeStates) {
        ...
        if (obj.native.LOCK_VALUE === undefined) {
            adapter.log.warn(
                `[ALEXA] Cannot choose value for lock: Please define in "${id}" the "native.LOCK_VALUE" with locking value`,
            );
            throw new Error('Cannot choose value for lock');
        } else {
            let state;
            ...
            if (
                ...
                obj.native.LOCK_VALUE === '1' ||
                obj.native.LOCK_VALUE === 1 ||
                obj.native.LOCK_VALUE === 'locked'
            ) {
                obj.native.LOCK_VALUE = true;
            }
            if (
                obj.native.LOCK_VALUE === 'false' ||
                obj.native.LOCK_VALUE === '0' ||
                obj.native.LOCK_VALUE === 0 ||
                obj.native.LOCK_VALUE === 'unlocked'
            ) {
                obj.native.LOCK_VALUE = false;
            }
    ...
    this.getDebug = function (cb) {
        ...
        for (let j = 0; j < discoveredAppliances.length; j++) {
            ...
            if (discoveredAppliances[j].additionalApplianceDetails.names !== undefined)
                delete discoveredAppliances[j].additionalApplianceDetails.names;
            if (discoveredAppliances[j].additionalApplianceDetails.name !== undefined)
                delete discoveredAppliances[j].additionalApplianceDetails.name;
            if (discoveredAppliances[j].additionalApplianceDetails.byON !== undefined)
                delete discoveredAppliances[j].additionalApplianceDetails.byON;
            if (discoveredAppliances[j].additionalApplianceDetails.byONs !== undefined)
                delete discoveredAppliances[j].additionalApplianceDetails.byONs;
            if (discoveredAppliances[j].additionalApplianceDetails.nameModified !== undefined)
                delete discoveredAppliances[j].additionalApplianceDetails.nameModified;
            if (discoveredAppliances[j].additionalApplianceDetails.room !== undefined)
                delete discoveredAppliances[j].additionalApplianceDetails.room;
            if (discoveredAppliances[j].additionalApplianceDetails.func !== undefined)
                delete discoveredAppliances[j].additionalApplianceDetails.func;
            if (discoveredAppliances[j].additionalApplianceDetails.smartType !== undefined)
                delete discoveredAppliances[j].additionalApplianceDetails.smartType;
            if (discoveredAppliances[j].additionalApplianceDetails.smartTypes !== undefined)
                delete discoveredAppliances[j].additionalApplianceDetails.smartTypes;
            if (discoveredAppliances[j].additionalApplianceDetails.channels !== undefined)
                delete discoveredAppliances[j].additionalApplianceDetails.channels;
        }
    ...
    this.process = async (request, isEnabled) => {
        ...
        if (
            request &&
            request.payload &&
            request.payload.appliance &&
            request.payload.appliance.additionalApplianceDetails
        ) {
            let details = null;
        ...
        switch (request.header.name) {
            ...
            case 'TurnOnRequest': {
                ...
                for (let channel in channels) {
                    ...
                    // try to find ON value for dimmer
                    ...
                    for (let i = 0; i < channels[channel].length; i++) {
                        ...
                        try {
                            ...
                            adapter.log.warn(`[ALEXA] Cannot controlOnOff ${err}`);
                        }
                    }
                }

                writeResponse(request.payload.appliance.applianceId, 'ONOFF', true).catch(err =>
                    adapter.log.error(`[ALEXA] ${err}`),
                );
            ...
            case 'TurnOffRequest': {
                ...
                writeResponse(request.payload.appliance.applianceId, 'ONOFF', false).catch(err =>
                    adapter.log.error(`[ALEXA] ${err}`),
                );
            ...
            case 'SetPercentageRequest': {
                adapter.log.debug(
                    `[ALEXA] ALEXA Percent: ${request.payload.appliance.applianceId} ${request.payload.percentageState.value}%`,
                );
                for (let channel in channels) {
                    if (!channels.hasOwnProperty(channel)) {
                        continue;
                    ...
                    for (let i = 0; i < channels[channel].length; i++) {
                        ...
                        try {
                            await controlPercent(
                                dimmer,
                                id,
                                request.payload.percentageState.value,
                ...
                writeResponse(request.payload.appliance.applianceId, '%', request.payload.percentageState.value).catch(
                    err => adapter.log.error(`[ALEXA] ${err}`),
                );
            ...
            case 'IncrementPercentageRequest': {
                adapter.log.debug(
                    `[ALEXA] ALEXA Increment: ${request.payload.appliance.applianceId} ${request.payload.deltaPercentage.value}%`,
                );
                for (let channel in channels) {
                    if (!channels.hasOwnProperty(channel)) {
                        continue;
                ...
                writeResponse(request.payload.appliance.applianceId, '<>', request.payload.deltaPercentage.value).catch(
                    err => adapter.log.error(`[ALEXA] ${err}`),
                );
            ...
            case 'DecrementPercentageRequest': {
                adapter.log.debug(
                    `[ALEXA] ALEXA decrement: ${request.payload.appliance.applianceId} ${request.payload.deltaPercentage.value}%`,
                );
                for (let channel in channels) {
                    if (!channels.hasOwnProperty(channel)) {
                        continue;
                    ...
                    for (let i = 0; i < channels[channel].length; i++) {
                        ...
                        try {
                            await controlDelta(
                                dimmer,
                                id,
                                request.payload.percentageState.value * -1,
                                Object.keys(channels).length === 1,
                            );
                        } catch (err) {
                            adapter.log.error(`[ALEXA] Cannot controlDelta: ${err}`);
                ...
                writeResponse(
                    request.payload.appliance.applianceId,
                    '<>',
                    -1 * request.payload.deltaPercentage.value,
            ...
            case 'SetTargetTemperatureRequest': {
                adapter.log.debug(
                    `[ALEXA] ALEXA temperature Percent: ${request.payload.appliance.applianceId} ${request.payload.targetTemperature.value} grad`,
                );
                let err;
                let response;
                for (let channel in channels) {
                    ...
                    for (let i = 0; i < channels[channel].length; i++) {
                        ...
                        try {
                            const _response = await controlTemperature(
                                id,
                                request.payload.targetTemperature.value,
                                Object.keys(channels).length === 1,
                            );
            ...
            case 'IncrementTargetTemperatureRequest': {
                ...
                adapter.log.debug(
                    `[ALEXA] ALEXA temperature Increment: ${request.payload.appliance.applianceId} ${request.payload.deltaTemperature.value} grad`,
                );
                let response;
                let err;
                ...
                for (let channel in channels) {
                    ...
                    for (let i = 0; i < channels[channel].length; i++) {
                        ...
                        try {
                            const _response = await controlTemperatureDelta(
                                id,
                                request.payload.deltaTemperature.value,
                                Object.keys(channels).length === 1,
                            );
                            if (_response && !response) {
                                response = _response;
            ...
            case 'DecrementTargetTemperatureRequest': {
                request.payload.deltaTemperature.value = request.payload.deltaTemperature.value || 1;
                adapter.log.debug(
                    `[ALEXA] ALEXA temperature decrement: ${request.payload.appliance.applianceId} ${request.payload.deltaTemperature.value} grad`,
                );
                let response;
                let err;
                ...
                for (let channel in channels) {
                    ...
                    for (let i = 0; i < channels[channel].length; i++) {
                        ...
                        try {
                            const _response = await controlTemperatureDelta(
                                id,
                                request.payload.deltaTemperature.value * -1,
                                Object.keys(channels).length === 1,
                            );
                            if (_response && !response) {
                                response = _response;
                ...
                writeResponse(
                    request.payload.appliance.applianceId,
                    '°<>',
                    -1 * request.payload.targetTemperature.value,
                ).catch(err => adapter.log.error(`[ALEXA] ${err}`));
            ...
            case 'SetColorRequest': {
                adapter.log.debug(
                    `[ALEXA] ALEXA Color: ${request.payload.appliance.applianceId} ${JSON.stringify(request.payload.color)}`,
                );
            ...
            case 'SetColorTemperatureRequest': {
                for (let channel in channels) {
                    ...
                    for (let i = 0; i < channels[channel].length; i++) {
                        ...
                        try {
                            await controlColorTemperature(
                                temperature,
                                id,
                                request.payload.colorTemperature.value,
                                Object.keys(channels).length === 1,
                            );
                        } catch (err) {
            ...
            case 'IncrementColorTemperatureRequest': {
                ...
                for (let channel in channels) {
                    ...
                    for (let i = 0; i < channels[channel].length; i++) {
                        ...
                        adapter.log.debug(`[ALEXA] Controlling temperature kelvin: ${id}`);
                        try {
                            const _res = await controlColorTemperatureDelta(
                                temperature,
                                id,
                                -1,
                                Object.keys(channels).length === 1,
                            ...
                            adapter.log.warn(`[ALEXA] Cannot controlColorTemperatureDelta: ${err}`);
                        }
                    }
                }
            ...
            case 'HealthCheckRequest': {
                ...
                try {
                    adapter.log.debug(
                        `[ALEXA] HealthCheckRequest duration: ${Date.now() - request.payload.initiationTimestamp} ms`,
                    );
                } catch (e) {
        ...
        adapter.getObjectView('system', 'state', {}, (err, _states) => {
            ...
            adapter.getObjectView('system', 'enum', {}, (err, doc) => {
                ...
                for (let f = 0; f < funcs.length; f++) {
                    if (
                        !funcs[f].common ||
                        !funcs[f].common.members ||
                        typeof funcs[f].common.members !== 'object' ||
                        !funcs[f].common.members.length
                    ) {
                        continue;
                    }
