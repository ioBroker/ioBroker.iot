 ...
 * @returns {Promise<{ result: 'Ok', message: ioBroker.Message }>}
 */
async function handleSendToAdapter(visuData, adapter) {
    const { instance, command, message } = visuData;

    const resp = await adapter.sendToAsync(instance, command, message, { timeout: 20_000 });
 ...
 * @param {ioBroker.Adapter} adapter the adapter instance
 * @returns {Promise<{ result: 'Ok', instances: string[] }>}
 */
