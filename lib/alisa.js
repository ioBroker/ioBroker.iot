...
class YandexAliceConverter {
    ...
    _getStateBooleanProperty(entity, name, mapping) {
        return new Promise(resolve => {
            ...
            const capability = entity.context.properties.find(cap => cap.type === 'devices.properties.event' && cap.parameters.instance === name);
            if (capability && stateId) {
                this.adapter.getForeignState(stateId, (err, state) => {
                    if (!err && state) {
                        capability.state = {
                            instance: name,
                            value: mapping[state.val === 'false' || state.val === '0' ? false : (state.val === 'true' || state.val === true || state.val === 1 || state.val === '1' || !!state.val)],
                        };
                    }
                    resolve();
                });
    ...
    _processButtonSensor(id, control, name, room, func, _obj, objects) {
        ...
        entity.COMMANDS.get_state = entity =>
            this._getStateBooleanProperty(entity, 'button', {true: 'click'});
