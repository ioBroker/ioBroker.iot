...
class YandexAliceConverter {
            ...
            if (capability && stateId) {
                this.adapter.getForeignState(stateId, (err, state) => {
                    if (!err && state) {
                        capability.state = {
                            instance: name,
                            value: attribute.states[state.val],
                        };
                    }
                    resolve();
                });
            ...
            if (capability && stateId) {
                this.adapter.getForeignState(stateId, (err, state) => {
                    if (!err && state) {
                        capability.state = {
                            instance: name,
                            value: (attribute.states[state.val] || '').toLowerCase(),
                        };
                    }
                    resolve();
                });
    ...
    _setStateMode(entity, data, name) {
        return new Promise(resolve => {
            const attribute = entity.ATTRIBUTES.find(attr => attr.attribute === name);
            const stateId = attribute ? attribute.setId : undefined;
            const capability = data.capabilities.find(cap => cap.type === 'devices.capabilities.mode' && cap.state.instance === name);
            if (capability && capability.state && stateId) {
                const val = Object.keys(attribute.states).find(val => attribute.states[val].toLowerCase() === capability.state.value.toLowerCase());
    ...
    _processAC(id, control, name, room, func, _obj, objects) {
        ...
        entity.COMMANDS.set_state = (entity, data) =>
            this._setStateOnOff(entity, data)
                ...
                .then(() => this._setStateThermostat(entity))
                .then(() => this._setStateSwing(entity))
                .then(() => this._setStateSpeed(entity));
    ...
    _processVacuumCleaner(id, control, name, room, func, _obj, objects) {
        ...
        if (state && state.id) {
            ...
            entity.ATTRIBUTES.push({attribute: 'cleanup_mode', getId: state.id, setId: state.id, states});
            this._addID2entity(state.id, entity);
            const modes = [];
            const values = Object.values(states).map(name => name.toLowerCase());
        ...
        if (state && state.id) {
            const states = objects[state.id].common.states || {};
            entity.ATTRIBUTES.push({attribute: 'work_speed', getId: state.id, setId: state.id, states});
            this._addID2entity(state.id, entity);
            const modes = [];
            const values = Object.values(states).map(name => name.toLowerCase());
        ...
        if (state && state.id) {
            const states = objects[state.id].common.states || {};
            entity.ATTRIBUTES.push({attribute: 'pause', getId: state.id, setId: state.id, states});
            this._addID2entity(state.id, entity);
        ...
        if (state && state.id) {
            const states = objects[state.id].common.states || {};
            entity.ATTRIBUTES.push({attribute: 'water_level', getId: state.id, states});
            this._addID2entity(state.id, entity);
        ...
        if (state && state.id) {
            const states = objects[state.id].common.states || {};
            entity.ATTRIBUTES.push({attribute: 'battery_level', getId: state.id, states});
            this._addID2entity(state.id, entity);
