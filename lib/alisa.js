...
class YandexAliceConverter {
    ...
    _getStateThermostat(entity) {
        return new Promise(resolve => {
            ...
            const capability = entity.context.capabilities.find(cap => cap.type === 'devices.capabilities.mode');
            if (capability && stateId) {
                this.adapter.getForeignState(stateId, (err, state) => {
                    if (!err && state) {
                        capability.state = {
    ...
    _setStateThermostat(entity, data) {
        return new Promise(resolve => {
            const thermostat = entity.ATTRIBUTES.find(attr => attr.attribute === 'thermostat');
            const stateId = thermostat ? thermostat.getId : undefined;
            const capability = data.capabilities.find(cap => cap.type === 'devices.capabilities.mode');
            if (capability && capability.state && stateId) {
    ...
    _processAC(id, control, name, room, func, _obj, objects) {
        // create main device
        const entity = this._processCommon(id, name, room, func, _obj, 'devices.types.thermostat.ac');
