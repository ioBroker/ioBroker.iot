...
class YandexAliceConverter {
        ...
        return new Promise(resolve => {
            ...
            if (capability && capability.state && stateId) {
                if (entity.STATE.type === 'number') {
                    this.adapter.setForeignState(stateId, capability.state.value ? (entity.STATE.max !== undefined ? entity.STATE.max : 100) : (entity.STATE.min !== undefined ? entity.STATE.min : 0));
                } else {
    ...
    _getStateTemperature(entity) {
        return new Promise(resolve => {
            const temperature = entity.ATTRIBUTES.find(attr => attr.attribute === 'temperature');
            const stateId = temperature ? temperature.getId : undefined;
            const capability = entity.context.capabilities.find(cap => cap.type === 'devices.capabilities.range' && cap.parameters.instance === 'temperature');
            if (capability && stateId) {
                this.adapter.getForeignState(stateId, (err, state) => {
                    if (!err && state) {
                        capability.state = {
    ...
    _setStateTemperature(entity, data) {
        return new Promise(resolve => {
            const temperature = entity.ATTRIBUTES.find(attr => attr.attribute === 'temperature');
            const stateId = temperature ? temperature.getId : undefined;
            const capability = data.capabilities.find(cap => cap.type === 'devices.capabilities.range' && cap.state.instance === 'temperature');
            if (capability && capability.state && stateId) {
                this.adapter.setForeignState(stateId, capability.state.value);
                capability.state.action_result = {status: 'DONE'};
            }
    ...
    _getStateThermostat(entity) {
        return new Promise(resolve => {
            const thermostat = entity.ATTRIBUTES.find(attr => attr.attribute === 'thermostat');
            const stateId = thermostat ? thermostat.getId : undefined;
            const capability = entity.context.capabilities.find(cap => cap.type === 'devices.capabilities.mode' && cap.parameters.instance === 'thermostat');
            if (capability && stateId) {
                this.adapter.getForeignState(stateId, (err, state) => {
                    if (!err && state) {
                        capability.state = {
                            instance: 'thermostat',
                            value: (thermostat.states[state.val] || '').toLowerCase(),
                        };
                    }
                    resolve();
                });
    ...
    _setStateThermostat(entity, data) {
        return new Promise(resolve => {
            const thermostat = entity.ATTRIBUTES.find(attr => attr.attribute === 'thermostat');
            const stateId = thermostat ? thermostat.getId : undefined;
            const capability = data.capabilities.find(cap => cap.type === 'devices.capabilities.mode' && cap.state.instance === 'thermostat');
            if (capability && capability.state && stateId) {
                const val = Object.keys(thermostat.states).find(val => thermostat.states[val].toLowerCase() === capability.state.value.toLowerCase());
                this.adapter.setForeignState(stateId, val);
                capability.state.action_result = {status: 'DONE'};
            }
            resolve(data);
    ...
    _getStateSwing(entity) {
        return new Promise(resolve => {
            const swing = entity.ATTRIBUTES.find(attr => attr.attribute === 'swing');
            const stateId = swing ? swing.getId : undefined;
            const capability = entity.context.capabilities.find(cap => cap.type === 'devices.capabilities.mode' && cap.parameters.instance === 'swing');
            if (capability && stateId) {
                this.adapter.getForeignState(stateId, (err, state) => {
                    if (!err && state) {
                        capability.state = {
                            instance: 'swing',
                            value: (swing.states[state.val] || '').toLowerCase(),
                        };
                    }
                    resolve();
                });
    ...
    _setStateSwing(entity, data) {
        return new Promise(resolve => {
            const swing = entity.ATTRIBUTES.find(attr => attr.attribute === 'swing');
            const stateId = swing ? swing.getId : undefined;
            const capability = data.capabilities.find(cap => cap.type === 'devices.capabilities.mode' && cap.state.instance === 'swing');
            if (capability && capability.state && stateId) {
                const val = Object.keys(swing.states).find(val => swing.states[val].toLowerCase() === capability.state.value.toLowerCase());
                this.adapter.setForeignState(stateId, val);
                capability.state.action_result = {status: 'DONE'};
            }
            resolve(data);
    ...
    _getStateSpeed(entity) {
        return new Promise(resolve => {
            const fan_speed = entity.ATTRIBUTES.find(attr => attr.attribute === 'fan_speed');
            const stateId = fan_speed ? fan_speed.getId : undefined;
            const capability = entity.context.capabilities.find(cap => cap.type === 'devices.capabilities.mode' && cap.parameters.instance === 'fan_speed');
            if (capability && stateId) {
                this.adapter.getForeignState(stateId, (err, state) => {
                    if (!err && state) {
                        capability.state = {
                            instance: 'fan_speed',
                            value: (fan_speed.states[state.val] || '').toLowerCase(),
                        };
                    }
                    resolve();
                });
    ...
    _setStateSpeed(entity, data) {
        return new Promise(resolve => {
            const fan_speed = entity.ATTRIBUTES.find(attr => attr.attribute === 'fan_speed');
            const stateId = fan_speed ? fan_speed.getId : undefined;
            const capability = data.capabilities.find(cap => cap.type === 'devices.capabilities.mode' && cap.state.instance === 'fan_speed');
            if (capability && capability.state && stateId) {
                const val = Object.keys(fan_speed.states).find(val => fan_speed.states[val].toLowerCase() === capability.state.value.toLowerCase());
                this.adapter.setForeignState(stateId, val);
                capability.state.action_result = {status: 'DONE'};
            }
            resolve(data);
    ...
    _getStateBlinds(entity) {
        return new Promise(resolve => {
            const dimmer = entity.ATTRIBUTES.find(attr => attr.attribute === 'open');
            const stateId = dimmer ? dimmer.getId : undefined;
            const capability = entity.context.capabilities.find(cap => cap.type === 'devices.capabilities.range');
            if (capability && stateId) {
                this.adapter.getForeignState(stateId, (err, state) => {
    ...
    _processDimmer(id, control, name, room, func, _obj, objects) {
        ...
        if (state && state.id) {
            setDimmer = state.id;
            entity.ATTRIBUTES.push({attribute: 'brightness', getId: getDimmer || setDimmer, setId: setDimmer});
            this._addID2entity(state.id, entity);
        } else if (getDimmer) {
            entity.ATTRIBUTES.push({attribute: 'brightness', getId: getDimmer, setId: null});
            this._addID2entity(state.id, entity);
        }
    ...
    _processCT(id, control, name, room, func, _obj, objects) {
         ...
         if (ctState && ctState.id) {
             ...
             entity.context.capabilities.push({
                 ...
                 parameters: {
                     temperature_k: {min: 2700, max: 9000, precision: 100},
                 },
             });
    ...
    _processAC(id, control, name, room, func, _obj, objects) {
        ...
        if (state && state.id) {
            ...
            entity.ATTRIBUTES.push({attribute: 'temperature', getId: getTemperature || setTemperature, setId: setTemperature});
            this._addID2entity(state.id, entity);
        } else if (getTemperature) {
            entity.ATTRIBUTES.push({attribute: 'temperature', getId: getTemperature, setId: null});
            this._addID2entity(state.id, entity);
        }
        ...
        if (getTemperature || setTemperature) {
            entity.context.capabilities.push({
                type: 'devices.capabilities.range',
                retrievable: !!getTemperature,
                parameters: {
                    instance: 'temperature',
                    unit: 'unit.temperature.celsius',
                    range: {min: 12, max: 30, precision: 1},
                },
            });
        }
        ...
        if (state && state.id) {
            ...
            entity.ATTRIBUTES.push({attribute: 'thermostat', getId: thermostatId, setId: thermostatId, states});
            this._addID2entity(state.id, entity);
            const modes = [];
        ...
        if (state && state.id) {
            ...
            entity.ATTRIBUTES.push({attribute: 'swing', getId: swingId, setId: swingId, states});
            this._addID2entity(state.id, entity);
            const modes = [];
            const values = Object.values(states).map(name => name.toLowerCase());
        ...
        if (state && state.id) {
            ...
            entity.ATTRIBUTES.push({attribute: 'fan_speed', getId: fanSpeedId, setId: fanSpeedId, states});
            this._addID2entity(state.id, entity);
            const modes = [];
            const values = Object.values(states).map(name => name.toLowerCase());
    ...
    _processBlinds(id, control, name, room, func, _obj, objects) {
        ...
        entity.COMMANDS.get_state = entity =>
            this._getStateOnOff(entity)
                .then(() =>
                    this._getStateBlinds(entity));

        entity.COMMANDS.set_state = (entity, data) =>
            this._setStateOnOff(entity, data)
                .then(res =>
                    this._setStateBlinds(entity, res));
            ...
            return {
                ...
                actions: device.context.capabilities.map(cap => {
                    ...
                    if (!typesMapping[capTextUnit]) {
                        this.adapter.log.debug('No mapping found for ' + capTextUnit)
                    }
