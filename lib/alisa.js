            ...
            this.keyPromise = new Promise((resolve, reject) => {
                ...
                request.get({
                    ...
                    if (!error && response && response.statusCode === 200) {
                        this.adapter.log.debug(`[ALISA] CHECK URL reported: ${JSON.stringify(body)}`);
                        this.urlKeyOk = true;
                        resolve();
                    } else {
                        this.adapter.config.yandexAlisa && this.adapter.log.error('[ALISA] Url Key error. Alisa Request and Response are working. But device states are not reported automatically. If you have pro license please try to delete iot.0.certs: ' + (error || body.error || body));
                        reject(error || body);
                    }
                });
            });
                ...
                .then(devices => {
                    ...
                    request.post({
                        ...
                        if (!error && response && response.statusCode === 200) {
                            ...
                            this.adapter.log.debug(`[ALISA] Status reported:  ${JSON.stringify(json)}  ${JSON.stringify(body)}`);
                        } else {
                            if (response && response.statusCode === 404) {
                                this.adapter.log.error(`[ALISA] device ${id} is unknown to google home`);
                                this.unknownDevices[id] = Date.now();
                            } else if (response && response.statusCode === 401) {
                                this.adapter.log.error(`[ALISA] auth error: ${JSON.stringify(body)}`);
                                this.urlKeyOk = false; // invalidate urlKey
                            } else if (response && response.statusCode === 410) {
                                this.adapter.log.error(`[ALISA] invalid protocol version: ${JSON.stringify(body)}`);
                                this.urlKeyOk = false; // invalidate urlKey
                            } else {
                                let errorMessage;
                                if (error) {
                                ...
                                this.adapter.log.debug(`[ALISA] ${JSON.stringify(json)}`);
                            }
                        }
                    });
                });
