...
class GoogleHome {
        ...
        if (!this.keyPromise || now - this.keyPromiseTime > 900000) {
            ...
            this.keyPromise = axios.get(url, {validateStatus: status => status === 200})
                ...
                .catch(error => {
                    ...
                    this.adapter.config.googleHome && this.adapter.log.error(`[GHOME] Url Key error. Google Request and Response are working. But device states are not reported automatically. If you have pro license please try to delete iot.0.certs: ${JSON.stringify(errorMessage)}`);
                });
        }
                    ...
                    if (controls) {
                        ...
                        if (this.converter.hasOwnProperty(control.type)) {
                            ...
                            if (result[id].customData.set_brightness) {
                                const briState = await this.adapter.getForeignObjectAsync(result[id].customData.set_brightness);
                                if (briState && briState.common && briState.common.max && briState.common.max >= 101) {
                                    result[id]['conv2GH'][result[id].customData.set_brightness] = 'return value/' + briState.common.max / 100;
                                    result[id]['conv2iob'][result[id].customData.set_brightness] = 'return value*' + briState.common.max / 100;
                                }
    ...
    async _updateDevices() {
        ...
        for (let i = 0; i < ids.length; i++) {
            ...
            if ((ids[i].startsWith('alias.') || ids[i].startsWith('linkeddevices.')) &&
                ...
                if (roomHint) {
                    await this.processState(ids, objects, ids[i], roomsT(this.lang, roomHint), funcsT(this.lang, 'undefined'), result, true);
                }
            }
        }
    ...
    async updateState(id, state) {
                        ...
                        try {
                            if (_attr === 'on') {
                                ...
                                if (!json[devId]['color_R'] && !!json[devId]['color_G'] && !json[devId]['color_B']) {
                                    ...
                                    const r = json[devId].color_R ? (await this.adapter.getForeignStateAsync(json[devId].color_R.id)) : {val: 0};
                                    const g = json[devId].color_G ? (await this.adapter.getForeignStateAsync(json[devId].color_G.id)) : {val: 0};
                                    const b = json[devId].color_B ? (await this.adapter.getForeignStateAsync(json[devId].color_B.id)) : {val: 0};
                                ...
                                const hue = json[devId].color_hue ? (await this.adapter.getForeignStateAsync(json[devId].color_hue.id)) : {val: 0};
                                const sat = json[devId].color_saturation ? (await this.adapter.getForeignStateAsync(json[devId].color_saturation.id)) : {val: 0};
                                const value = json[devId].brightness ? (await this.adapter.getForeignStateAsync(json[devId].brightness.id)) : {val: 0};
                                const h = hue.val / 360;
                                const s = sat.val / 255;
                            ...
                            if (this.smartDevices[devId].conv2GH[custom[attr]]) {
                                try {
                                    ...
                                    this.adapter.log.error(`Invalid convert function in ${devId}/${custom[attr]}: ${this.smartDevices[devId].conv2GH[custom[attr]]}`);
                                }
                            }
                            ...
                            if ((this.reportedStates[devId] && this.reportedStates[devId][_attr] && this.reportedStates[devId][_attr].val !== val) ||
                                !this.reportedStates[devId] || !this.reportedStates[devId][_attr]
                            ) {
        ...
        if (found && this.urlKey) {
            ...
            axios.post(url, json, {validateStatus: status => status === 200})
                .then(response => {
                    if (this.unknownDevices[id]) {
                        delete this.unknownDevices[id];
                    }
                    this.adapter.log.debug(`[GHOME] Status reported:  ${JSON.stringify(json)}  ${JSON.stringify(response.data)}`);
                })
                .catch(error => {
                    if (error.response && error.response.status === 404) {
                        ...
                        this.adapter.log.error(`[GHOME] invalid protocol version: ${JSON.stringify(error.response.body)}`);
                        this.urlKeyOk = false; // invalidate urlKey
                    } else {
                        let errorMessage;
                        if (error.response) {
                    ...
                    if (typeof json[devId][attr] === 'object' && json[devId][attr].id) {
                        ...
                        if (this.smartDevices[devId].conv2GH && this.smartDevices[devId].conv2GH[json[devId][attr].id]) {
                            try {
                                ...
                                this.adapter.log.error(`Invalid convert function in ${devId}/${attr}: ${this.smartDevices[devId].conv2GH[json[devId][attr].id]}`);
                            }
                        }
                        if (attr === 'on') {
            ...
            axios.post(url, json, {validateStatus: status => status === 200})
                ...
                .catch(error => {
                    if (error.response && error.response.status === 404) {
                        ...
                        this.adapter.log.error(`[GHOME] invalid protocol version: ${JSON.stringify(error.response.data)}`);
                        this.urlKeyOk = false; // invalidate urlKey
                    } else {
                        let errorMessage;
                        if (error.response) {
    ...
    sync(requestId) {
        ...
        if (devicesArray.length === 0) {
            this.adapter.log.warn('[GHOME] No devices defined. Did you add no sensors or indicate states to rooms and enums?');
        }
                            ...
                            if (attrs) {
                                Object.keys(attrs).forEach(attr => {
                                    if (attr && attr.startsWith('get_')) {
                                        if (this.smartDevices[dev.id].conv2GH && this.smartDevices[dev.id].conv2GH[attrs[attr]]) {
                                            try {
                                                const conv = new Function('value', this.smartDevices[dev.id].conv2GH[attrs[attr]]);
                                                states[attrs[attr]] = conv(states[attrs[attr]]);
                                            } catch (e) {
                                                this.adapter.log.error(`Invalid convert function in ${dev.id}/${attrs[attr]}: ${this.smartDevices[dev.id].conv2GH[attrs[attr]]}`);
                                            }
                                        }
                                        ...
                                        if (attrs[`set_${element.path}`]) {
                                            if (this.smartDevices[dev.id].conv2iob && this.smartDevices[dev.id].conv2iob[attrs[`set_${element.path}`]]) {
                                                try {
                                                    ...
                                                    this.adapter.log.error(`Invalid convert function in ${dev.id}/${attrs[`set_${element.path}`]}: ${this.smartDevices[dev.id].conv2iob[attrs[`set_${element.path}`]]}`);
                                                }
                                            }
                                            tasks.push({
