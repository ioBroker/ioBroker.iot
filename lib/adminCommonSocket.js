...
function updateLicenses(adapter, login, password, adminObj) {
    if (adapter.supportsFeature('CONTROLLER_LICENSE_MANAGER')) {
        return new Promise(async (resolve, reject) => {
            ...
            sendToHost(adapter, adminObj.common.host, 'updateLicenses', {login, password})
                .then(result => {
                    if (timeout) {
...
function getCompactInstalled(adapter, host) {
    return sendToHost(adapter, host, 'getInstalled', null)
        .then(data => {
            const result = {};
...
function getCompactRepository(adapter, host) {
    return sendToHost(adapter, host, 'getRepository', null)
        .then(data => {
...
function readLogs(adapter, host) {
    return new Promise((resolve, reject) => {
        ...
        sendToHost(adapter, host, 'getLogFiles', null)
            .then(result => {
                clearTimeout(timeout);
                timeout = null;
                result.error ? reject(result.error) : resolve(result.list);
...
async function getListOfAllAdapters(adapter) {
    ...
    if (!adapter.config.remoteAdminInstance || objects['system.adapter.' + adapter.config.remoteAdminInstance].common.auth) {
        // delete all admin links
        list = list.filter(item => item.adapter !== 'admin');
    }

    if (!adapter.config.remoteWebInstance || objects['system.adapter.' + adapter.config.remoteWebInstance].common.auth) {
        // delete all admin links
        list = list.filter(item => item.adapter === 'admin');
    }
