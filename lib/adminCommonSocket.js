    ...
    if (adapterConfig.native.accessLimit) {
        ...
        if (doc && doc.rows.length) {
            for (let i = 0; i < doc.rows.length; i++) {
                ...
                if (!obj.common.noConfig) {
                    await _readInstanceConfig(obj._id.substring('system.adapter.'.length), 'system.user.admin', false, configs);
                }
            }
        }
...
async function getListOfAllAdapters(adapter, lang, adminInstance) {
    // read all instances
