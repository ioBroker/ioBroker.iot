...
class RemoteAccess {
    ...
    _sendEvent(message, _originalMessage) {
        return axios.post('https://remote-events.iobroker.in/', message, { validateStatus: status => status === 200, timeout: 5000 })
            .then(response => {
    ...
    uploadToServer(url, data) {
        return axios.put(url, Buffer.from(data.file, 'base64'), {
            headers: {
    ...
    process(request, serviceType) {
        ...
        if (message) {
            ...
            if (_type === MESSAGE_TYPES.HTML) {
                ...
                if (name === 'listOfPrograms' || name === '/listOfPrograms') {
                ...
                if (name === 'vendorPrefix' || name === '/vendorPrefix') {
                ...
                return promiseFile
                    .then(data => {
                        ...
                        if (data.sid) {
                            ...
                            if (typeof args === 'string' && args?.startsWith('https:') && packed.length > MAX_IOT_MESSAGE_LENGTH) {
                                // upload file to temp server
                                return this.uploadToServer(args, data)
                                    .then(done => ({sid: message.sid, d: [_type, id, '', done ? '_$%URL' : 'Cannot upload']}))
                            }
