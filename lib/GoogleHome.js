...
class GoogleHome {
    ...
    processState(ids, objects, id, roomName, funcName, result, smartEnum, parentId) {
        try {
            ...
            if (objects[id].type === 'channel') {
                ...
                let childStates = this.getAllStatesInDevice(Object.keys(objects), id);
            ...
            if (typeof smartName === 'object' && smartName) {
                if(smartName.ghConv2GH) {
                    result[id].conv2GH = smartName.ghConv2GH
                }
                if(smartName.ghConv2iob) {
                    result[id].conv2iob = smartName.ghConv2iob
                }
            }
            ...
            if (this.smartDevicesSentToGoogle.hasOwnProperty(devId)) {
                ...
                for (const attr in custom) {
                    if (custom.hasOwnProperty(attr) && custom[attr] === id && attr.startsWith('get_')) {
                        ...
                        if (this.smartDevicesSentToGoogle[devId].conv2GH) {
                            const conv =  new Function("value", this.smartDevicesSentToGoogle[devId].conv2GH);
                            val = conv(val);
                        }
                                ...
                                Object.keys(execute.params).forEach(param => {
                                    ...
                                    paths.forEach(element => {
                                        if (attrs['set_' + element.path]) {
                                            ...
                                            if (this.smartDevices[dev.id].conv2iob) {
                                                const conv =  new Function("value", this.smartDevices[dev.id].conv2iob);
