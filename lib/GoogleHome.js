...
class GoogleHome {
    ...
    processState(ids, objects, id, roomName, funcName, result) {
        ...
        if (typeof friendlyName === 'object' && friendlyName) {
            result[id].type = friendlyName.ghType;
            if (friendlyName.ghTraits) {
                result[id].traits = friendlyName.ghTraits
            }
        }
        if (typeof friendlyName === 'object' && friendlyName) {
        ...
        if (!friendlyName || friendlyName === 'ignore' || friendlyName === false) {
            return;
        }

        const friendlyNames = friendlyName.replace(/, /g, ',').split(",");
        ...
        if (result[id].traits && result[id].traits.indexOf("Light") !== -1) {
            result[id].customData["set_brightness"] = true;
            result[id].customData["get_brightness"] = true;
    ...
    sync(requestId) {
        ...
        // let mergedDevices = [];
        // devices.forEach((element, mainIndex) => {
        //     mergedDevices.push(element)
        //     for (let index = mainIndex + 1; index < devices.length; index++) {
        //         const subElement = array[index];
        //         if (element.name.name === subElement.name.name && element.roomHint === subElement.roomHint && element.type === subElement.type) {
