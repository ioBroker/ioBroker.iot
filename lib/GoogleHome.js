...
class GoogleHome {
    constructor(adapter, urlKey) {
        ...
        this.getDevices()
            .then(smartDevices => this.smartDevices = smartDevices);
    }
    ...
    updateState(id, state) {
        if (false) {
            const url = `https://cplkn35nja.execute-api.eu-west-1.amazonaws.com/default/googeHomeStatus?user=${encodeURIComponent(this.adapter.config.login)}&urlKey=${encodeURIComponent(this.urlKey)}`;
            const json = {
                "hm-rpc.0.BLABLA103": {
                    on: true
                },
    ...
    sync(requestId) {
        const devices = Object.keys(this.smartDevices).map(id => this.smartDevices[id]);
