...
class GoogleHome {
    ...
    processState(ids, objects, id, roomName, funcName, result) {
        ...
        if (smartName === undefined && objects[id].common.write === false) {
            return;
        } else if (funcName && roomName) {
            friendlyName = funcName + " " + roomName;
        }


        this.adapter.log.debug(JSON.stringify(friendlyName))
        result[id] = {};
        result[id].traits = [];
        ...
        result[id].name = {
            ...
            this.adapter.log.debug("set traits")
        if (smartName && smartName.ghTraits && smartName.ghTraits.length > 0 && smartName.ghTraits[0] && traitEnum[smartName.ghTraits[0]]) {
            ...
            this.adapter.log.debug("check if default attributes")
        ...
        this.adapter.log.debug("start merging")
        ...
        if (this.smartNames[friendlyName] && this.smartNames[friendlyName].roomHint === roomName) {
            if (smartName && smartName.ghTraits && Array.isArray(smartName.ghTraits) && smartName.ghTraits.length > 0 && smartName.ghTraits[0]) {
                ...
                this.adapter.log.debug(`merging ${id} ind ${orgId}`)
    ...
    _updateDevices() {
        return this._readObjects()
            .then(data => {
                ...
                let result = {};
                let roomNames = {};
                this.adapter.log.debug(`Start smart enum processing`)
                funcs.forEach(funcId => {
                    ...
                    func.common.members.forEach(id => {
                        rooms.forEach(roomId => {
                            ...
                            this.adapter.log.debug(`Process ${roomId}`)
                ...
                Object.keys(objects).forEach(id => {
                    ...
                    if (smartName && smartName !== 'ignore' && smartName !== false) {
                        ...
                        allRooms.forEach(roomId => {
                            ...
                            // If state or channel is in some room and in some function
                            const pos = room.common.members.indexOf(id);
                            this.adapter.log.debug(`Check in room ${roomId}`)
                            if (pos !== -1) {
                                // Get the name of function (with language and if name is empty)
                                let roomName = this.getSmartName(room);
                                roomName = roomName || room.common.name;
    ...
    sync(requestId) {
        ...
        const devicesTemp = Object.keys(this.smartDevices).map(id => {
            if (!this.smartDevices[id].merged) {
                if (this.smartDevices[id].name.name && this.smartDevices[id].type && this.smartDevices[id].traits && this.smartDevices[id].traits.length > 0) {
