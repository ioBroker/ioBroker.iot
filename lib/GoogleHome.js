...
class GoogleHome {
        ...
        if (!id) {
            if (!this.adapter.config.noCommon) {
                if (states.common) {
                    ...
                    this.adapter.log.warn("No common property for " + JSON.stringify(states) +" IoT options available please check the state.")
                    return null;
                }
                ...
                try {
                    ...
                    if (controls) {
                        ...
                        if (this.converter.hasOwnProperty(control.type)) {
                            ...
                            // if(id.indexOf("20f81ce.temperature") !== -1) {
                            //     debugger
                            // }
                            ...
                            if (result[id].customData.set_color_temperature) {
                                ...
                                if (set_color_temperature && set_color_temperature.common && set_color_temperature.common.max <= 500) {
                                    result[id]["conv2GH"][result[id].customData.set_brightness] = "return 1000000/value"
                                    result[id]["conv2iob"][result[id].customData.set_brightness] = "return 1000000/value"
                                }
                            }
            ...
            this._updateDevices()
                .then(smartDevices => {
                    ...
                    this.checkUrlKey()
                        ...
                        .catch(err => {
                            this.adapter.config.googleHome && this.adapter.log.warn('[GHOME] Invalid URL Pro key. Status autoupdate is disabled you can set states but receive states only manually: ' + err)
                            ...
                            rooms.forEach(roomId => {
                                ...
                                if (pos !== -1) {
                                    ...
                                    if(objects[id] && objects[id].type === "state") {
                                        this.adapter.log.warn(id + " is a state. It's recommended to add rooms and functionality to channels or devices and not to a state to get autodetected for Google Home. This works only for simple switches.")
                                    }
                        ...
                        if (_attr === 'on') {
                           ...
                           val = Math.floor(r << 16 + g << 8 + b);
                            ...
                            if (rgb && rgb.substring) {
                                val = parseInt(rgb.substring(1), 16);
                            } else {
                              val = rgb;
                            }
        ...
        for (let devId in json) {
            ...
            for (let attr in json[devId]) {
                ...
                if (typeof json[devId][attr] === 'object' && json[devId][attr].id) {
                    this.adapter.getForeignState(json[devId][attr].id, async (err, state) => {
                        ...
                        if (attr === 'on') {
                            ...
                            json[devId].color =  json[devId].color ?  json[devId].color : {};
                            if (rgb && rgb.substring) {
                                json[devId].color.spectrumRGB = parseInt(rgb.substring(1), 16);
                            } else {
                            ...
                            json[devId][attr] = state.val ? true:false;
    ...
    sync(requestId) {
        ...
        this.tasksTimer = setTimeout(() => this.updateStates(), 5000);
        ...
        return new Promise(resolve => {
            ...
            devices.forEach(dev => {
                if (this.smartDevices[dev.id]) {
                    ...
                    if (attrs) {
                        Object.keys(attrs).forEach(attr => attr.startsWith("get_") && attrs[attr] !== undefined && ids.indexOf(attrs[attr]) === -1 && ids.push(attrs[attr]));
                    } else {
                        this.adapter.log.error("No Google Home customData for " + dev.id + " cannot handle Google Request.");
                    }
                }
            });
            this.getStates(ids, states => {
                ...
                this.adapter.log.error(JSON.stringify(devices))
                this.adapter.log.error(ids)
