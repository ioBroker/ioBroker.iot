...
class GoogleHome {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.get({
                ...
                if (!error && response.statusCode === 200) {
                    ...
                    this.adapter.config.googleHome && this.adapter.log.error('[GHOME] Url Key error. Google Request and Repsonse are working. But cannot report device state automatically: ' + (error || body.error || body));
                    reject(error || body);
                }
            });
        });
        ...
        if (!name) {
            ...
            if (room) {
                ...
                if (pos !== -1) {
                    ...
                    _name = _name.substring(0, pos) + _name.substring(pos + room.length + 1);
                }
            }
            if (func) {
    ...
    processWindowTilt(id, control, name, room, func, obj) {
        ...
        const get_lock = (getWindowTile && getWindowTile.id);
    ...
    processThermostat(id, control, name, room, func, obj) {
        ...
        return {
            ...
            displayTraits: ["action.devices.traits.TemperatureSetting_thermostatTemperatureSetpoint"],
            attributes: {
                "availableThermostatModes": "off,heat,cool,dry,auto,purifier,on",
                "thermostatTemperatureUnit": "C"
            },
            displayAttributes: JSON.stringify({
                "availableThermostatModes": "off,heat,cool,dry,auto,purifier,on",
    ...
    processMedia(id, control, name, room, func, obj) {
        ...
        const set_volumeLevel = (setVolumeState && setVolumeState.id);
        ...
        return {
            ...
            traits,
            displayTraits: ['action.devices.traits.Volume'],
            name: {
            ...
            this._updateDevices()
                .then(smartDevices => {
                    ...
                    this.checkUrlKey()
                        ...
                        .catch(err => this.adapter.config.googleHome && this.adapter.log.warn('[GHOME] Invalid URL key. Status update is disabled you can set states but not receive state updates: ' + err));
                });
        });
    }
                    ...
                    command.devices.forEach(dev => {
                        if (this.smartDevices[dev.id]) {
                            ...
                            if (execute.command === 'action.devices.commands.SetToggles') {
                                ...
                                if (typeof execute.params === "undefined" && execute.command) {
