                        ...
                        if (this.converter.hasOwnProperty(control.type)) {
                            ...
                            Object.keys(result[id].customData).forEach(element => {
                                ...
                                if (result[childID] && !result[childID].smartEnum) {
                                    ...
                                    this.adapter.log.warn(childID + ' has custom settings this is overriding auto-detected settings. To use auto-detected settings please delete this state in the instance settings under Google Devices.');
                                    return;
                                }
                ...
                for (const attr in custom) {
                    if (custom.hasOwnProperty(attr) && attr && custom[attr] === id && attr.startsWith('get_')) {
                        ...
                        try {
                            ...
                            this.adapter.log.error(error.message + ' ' + error.stack)
                        }
                    }
                }
            }
    ...
    query(requestId, devices) {
        return new Promise(resolve => {
            ...
            try {
                ...
                this.adapter.log.error((ids || []).toString());
            }
        });
    }
                ...
                command.execution.forEach(execute => {
                    ...
                    command.devices.forEach(dev => {
                        if (this.smartDevices[dev.id]) {
                            ...
                            if (!attrs) {
                                return this.adapter.log.error(`Try to control ${dev.id}, but the device is not configured for Google Home`);
                            }
