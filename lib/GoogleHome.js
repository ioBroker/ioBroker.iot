...
class GoogleHome {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.get({
                ...
                if (!error && response.statusCode === 200) {
                    ...
                    this.adapter.config.googleHome && this.adapter.log.error('[GHOME] Url Key error Cannot report device state: ' + (error || body.error || body));
                    reject(error || body);
                }
            });
        });
    ...
    processTemperature(id, control, name, room, func, obj) {
        ...
        return {
            ...
            traits,
            displayTraits:["action.devices.traits.TemperatureSetting_thermostatTemperatureAmbient"],
            attributes: {
    ...
    processState(ids, objects, id, roomName, funcName, smartEnum, result) {
        try {
            ...
            if (typeof smartName === 'object' && smartName && objects[id].type !=="channel"  && objects[id].type !=="device" ) {
                if (smartName.ghType) {
                ...
                try {
                    ...
                    if (controls) {
                        ...
                        const control = controls[0] //controls.forEach(control => {
        ...
        return new Promise(resolve => {
            ...
            commands.forEach(command => {
                command.execution.forEach(execute => {
                    console.log(`${execute.command} => ${execute.params}`);
