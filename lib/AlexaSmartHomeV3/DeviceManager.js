...
class DeviceManager {
    ...
    async collectEndpoints() {
        ...
        while (detectedControls.length) {
            ...
            if (control.room?.common?.name) {
                if (control.functionality?.common?.name) {
                    ...
                    this.toDevice(processedControls, Utils.friendlyNameByRoomAndFunctionName(control));
        ...
        const stateIds = this.devices.flatMap(d => d.controls).flatMap(item => item.supported).flatMap(item => item.properties).map(item => item.stateProxy.getId);
        ...
        if (failedReasons.length) {
            this.log.debug(`failed to subscibe for updates of ${failedReasons.length} states`);
            try {
    ...
    async handleAlexaEvent(event) {
        ...
        if (directive) {
            ...
            if (device) {
                ...
                response = AlexaResponse.endpointUnreachable();
            }
        }
    ...
    async handleStateUpdate(id, state) {
        ...
        for (const device of this.devices) {
            ...
            const supported = device.controls.flatMap(item => item.supported).flatMap(item => item.properties).find(item => item.stateProxy.getId === id);
            ...
            if (supported) {
                ...
                if (supported.stateProxy.currentValue === state.val) {
                    ...
                    const event = Directives.ChangeReport.get(device.id, supported.property.propertyName, true);
                    const directive = this.matchDirective(event);
                    if (directive) {
                        const stateChange = await directive.handle(event, this);
                        this.publishStateChange(stateChange);
