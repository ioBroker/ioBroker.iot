...
class DeviceManager {
    ...
    toDevice(detectedControls, friendlyName, autoDetected) {
        let controls = []
    ...
    async collectEndpoints() {
        ...
        while (detectedControls.length) {
            ...
            if (control.room?.common?.name) {
                if (control.functionality?.common?.name) {
                    ...
                    this.toDevice(processedControls, Utils.friendlyNameByRoomAndFunctionName(control, this.lang), true);
                } else {
                ...
                this.toDevice([control], Utils.stringify(control.object.common?.smartName, this.lang), control.autoDetected);
        ...
        const stateIds = new Set(this.devices.flatMap(d => d.controls).flatMap(item => item.supported).flatMap(item => item.properties).map(item => item.getId).filter(id => id));
        this.log.debug(`registering for updates of total ${stateIds.size} states`);
        const promises = [];
        // subscribe to updates
        for (const id of Array.from(stateIds)) {
