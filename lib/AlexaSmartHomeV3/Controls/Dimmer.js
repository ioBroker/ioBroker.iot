...
class Dimmer extends Control {
    ...
    valueSetter(event) {
        if (Capabilities.BrightnessController.matches(event)
            && Capabilities.BrightnessController.directive(event) === Capabilities.BrightnessController.ADJUST) {
        ...
        if (capability.name === Capabilities.BrightnessController.name) {
            return new StateProxy({
                ...
                alexaSetter: function (alexaValue) {
                    return Utils.denormalize_0_100(alexaValue, valuesRangeMin, valuesRangeMax);
                },
                alexaGetter: function (value) {
                    return Utils.normalize_0_100(value, valuesRangeMin, valuesRangeMax);
                }
            })
        }
    }
