...
class Control {
     ...
     * and at least one property. Every property has a pre-configured StateProxy with iobroker state ids and value converters from Alexa to iobroker types
     * and vice versa.
     *
     ...
     * Every capability has at least one property with a pre-configured StateProxy with iobroker state ids and value converters from Alexa to iobroker types
     * and vice versa.
     *
     * @param {*} ctrl - a control in terms of iobroker type detector.
     * @returns Array of objects with natively supported Alexa capabilities and correspondingly configured instances of StateProxies
    ...
    async setValue(event, eventHandler) {
        ...
        const alexaValue = eventHandler.property.alexaDirectiveValue(event);
    ...
    async adjustValue(event, eventHandler) {
        ...
        const delta = eventHandler.property.alexaDirectiveValue(event);
        // get current value
