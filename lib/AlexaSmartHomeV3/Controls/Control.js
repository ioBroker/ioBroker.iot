...
class Control {
    ...
    async handle(event) {
        ...
        if (property) {
            ...
            const response = AlexaResponse.handled(event, property.propertyName, property.reportValue(alexaValue));
    ...
    states(ctrl) {
        ...
        for (const stateKey of this.stateKeys) {
            result[stateKey] = ctrl.states.find(s => s.name === stateKey);
        }
        return result;
    }
    ...
    statesMap() {
        const result = {}
        for (const stateKey of this.stateKeys) {
            result[stateKey.toLowerCase()] = stateKey;
        }
        return result;
    }
    ...
    blankInitObject(ctrl) {
        const states = this.states(ctrl);
        const map = this.statesMap();
        return {
            setState: states[map.actual],
            getState: states[map.actual],
        };
    }

    powerStateInitObject(ctrl) {
        const states = this.states(ctrl);
        const map = this.statesMap();
