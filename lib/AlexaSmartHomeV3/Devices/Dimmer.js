...
/**
 * iobroker 'light' and 'dimmer' devices are merged to the same device if they're in the same room,
 ...
 * but the current state of the 'dimmer' is 0. 
...
class Dimmer extends Device {
    ...
    // async handle(event) {
    //     // find out the capability to handle the Alexa directive
    //     const capability = this.matchCapability(event);
    //     // handle
    //     let response = await capability.handle(event);

    //     // custom logic to update devices' states not supporting this capability
    //     if (capability.namespace === this.BrightnessController) {
    //         const powerController = this.capabilityByNamespace(this.PowerController)
    //         if (powerController) {
    //             const alexaValue = capability.alexaValue(event);
    //             // get

    //         }
    //     } else if (capability.namespace === this.PowerController) {
