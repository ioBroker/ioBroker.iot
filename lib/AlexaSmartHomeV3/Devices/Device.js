...
class Device {
    constructor(opts) {
        ...
        this.endpoints = Helpers.defaultIfNullOrEmpty(opts.endpoints, []);
    }

    async handle(event) {
        const promises = []
        this.endpoints.forEach(endpoint => promises.push(endpoint.matchCapability(event).handle(event)))
        const results = await Promise.allSettled(promises);
        return results[0];
    }

    get capabilities() {
        return this.endpoints[0].capabilities;
    }
}
