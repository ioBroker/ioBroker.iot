...
const DeviceAggregator = require('./DeviceAggregator');
...
module.exports = {
    getDevices: async function (adapter) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            // aggregate devices
            let aggregator = new DeviceAggregator(adapter, lang)

            // build a state map
            let stateMap = await allStatesAsMap(adapter);
            // build an enum map
            let enumMap = await allEnumsAsMap(adapter);
            // make all states aware of the rooms and functions they belong to
            enumMap.forEach((element, key) => {
                element.value.common.members.forEach((/** @type {string} */ id) => {
                    let state = stateMap.get(id);

                    if (!state) {
                        if (id.startsWith('alias')) {
                            // find all alias'es children
                            let keysIterator = stateMap.keys();
                            for (let k = keysIterator.next(); !k.done; k = keysIterator.next()) {
                                if (k.value.startsWith(id + '.')) {
                                    state = stateMap.get(k.value);
                                    if (state) {
                                        if (element.isRoom) {
                                            state.rooms.push(key);
                                        } else {
                                            state.functions.push(key);
                                        }
                                    }
                                }
                            }
                        ...
                        if (element.value.isRoom) {
                            state.value.rooms.push(key);
                        } else {
                            state.value.functions.push(key);
                        }
                    }
                });
            });

            // // at this point any state featuring a smartName or being member of a room or function is an Alexa device
            // let devices = []
            // stateMap.forEach(state => {
            //     if (state.smartName || state.rooms.length || state.functions.length) {
