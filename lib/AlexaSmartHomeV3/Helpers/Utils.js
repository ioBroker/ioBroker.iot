...
let functionalitiesAndRooms = async function (adapter) {
    ...
    const notEmptyRoomsAndFunctionalities = enumerations.filter(item => item?.common?.smartName !== false && item?.common?.smartName !== 'ignore').filter(item => item?.common?.members?.length);
...
module.exports = {
    ...
    controls: async function (adapter, lang) {
        ...
        keys.forEach(id => {
            ...
            if (this.isValidSmartName(devicesObject[id].common?.smartName, lang)) {
                // convert the old smartName format to the new one
                // `smartName: "Spot clean"` => `smartName: {"en": "Spot clean"}`
                if (typeof devicesObject[id].common.smartName === 'string') {
                    devicesObject[id].common.smartName = {[lang]: devicesObject[id].common.smartName};
                }
        ...
        for (let s = 0; s < idsWithSmartName.length; s++) {
            ...
            if (patterns[common.smartName.smartType]) {
                ...
                if (state) {
                    ...
                    const name = common.smartName[lang] || common.smartName.en || common.smartName.de;
