...
class RateLimiter {
    ...
    static async incrementAndGet(endpointId) {
        ...
        if (!Utils.isCurrentHour(new Date(item.timestamp))) {
        ...
        try {
            ...
            console.error(`Cannot store usage data: ${error}`)
        }
        ...
        const changesToday = Array.from(this.usage.values()).filter(item => Utils.isToday(new Date(item.timestamp))).reduce((sum, { changeCounter }) => sum + changeCounter, 0);
