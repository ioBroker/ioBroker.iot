...
module.exports = {
    ...
    /**
     * Inspects all objects (states, channels and devices) and tries to identify devices
     * considering directly assigned smart names and configured room and function enumerations.
     * To identify the devices the ioBroker type detector library is used (https://github.com/ioBroker/ioBroker.type-detector).
     * @param adapter The iot adapter instance
     * @returns {Promise} A promised object containing detected smart states/channels/devices
     */
    devices: async function (adapter) {
        ...
        functions.forEach(functionEnumItem => {
        ...
        // if a state has got a smart name directly assigned and its channel is not yet in the list, add its own id to the inspection list
        keys.forEach(id => {
            ...
            if (controls) {
                controls = controls.forEach(control => {
                    ...
                    if (control.states.find(state => state.id)) {
                        ...
                        devicesObject[id].rooms = []
                        rooms.forEach(room => {
                            if (room?.common?.members.includes(id)) {
                                devicesObject[id].rooms.push(room)
                            }
                        });
