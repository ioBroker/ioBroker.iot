...
module.exports = { 
    ...
    async setState(id, value) {
        return await Adapter.get().setForeignStateAsync(id, value, false);
    },    
    async getState(id) {
        const state = await Adapter.get().getForeignStateAsync(id);
        return state.val;
    },
    /**
    ...
    */
    endpointName: function (controls, lang, funcNameFirst, concatination) {
        ...
        if (roomName) {
            ...
            name = `${funcNameFirst ? funcName : roomName}${funcName !== '' && concatination ? (' ' + concatination + ' ') : ''}${funcNameFirst ? roomName : funcName}`;
