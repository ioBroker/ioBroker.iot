...
class AlexaSH3 {
    ...
    getDevices() {
        ...
        return endpoints.map(endpoint => ({
            controls: endpoint.controls.map(control => {
                ...
                supported = supported
                    .map(prop => prop.replace(/^[A-Z]/, c => c.toLowerCase()))
                    .sort();
                ...
                // always sort by id, so the GUI can address the group by the first control
                .sort((c1, c2) => c1.type > c2.type ? -1 : (c1.type < c2.type ? 1 : 0)),
    ...
    async destroy() {
        ...
        await AdapterProvider.get().adapter.unsubscribeForeignObjectsAsync('enum.rooms.*');
    }
}
