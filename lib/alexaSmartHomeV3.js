...
function AlexaSH3(adapter) {
    ...
    function writeResponse(channelId, command, value) {
        ...
        adapter.log.warn('[ALEX3] Unknown applianceId: ' + channelId);
    }
    ...
    function processState(states, id, channels) {
        try {
            ...
            if (!states[id].native || !states[id].common) {
                adapter.log.debug('[ALEX3] Invalid state "' + id + '". Not exist or no native part.');
                return null;
            }
            ...
            for (let i = friendlyNames.length - 1; i >= 0; i--) {
                ...
                if (!friendlyNames[i]) {
                    ...
                    friendlyNames[i] = friendlyNames[i].substring(0, 128).replace(/[^.a-zA-Z0-9äÄüÜöÖßÉéÈèÀàÂâÊêÙùÛûÇçÎîËëÏïŸÿÔôŒœ]+/g, ' ');
                }
            }

            if (!friendlyNames[0]) {
                adapter.log.warn('[ALEX3] State ' + id + ' is invalid.');
                return;
            }
            ...
            if (states[id].common.write === false) {
                adapter.log.debug('[ALEX3] Name "' + friendlyDescription + '" cannot be written and will be ignored');
                return;
            }
            */

            friendlyDescription = friendlyDescription.substring(0, 128).replace(/[^.a-zA-Z0-9äÄüÜöÖßÉéÈèÀàÂâÊêÙùÛûÇçÎîËëÏïŸÿÔôŒœ]+/g, ' ');
            ...
            for (let n = 0; n < friendlyNames.length; n++) {
                if (!channels[channelId]) {
                    ...
                    if (typeof channels[channelId].cookie.name === 'object') {
                        channels[channelId].cookie.name = channels[channelId].cookie.name[lang] || channels[channelId].cookie.name.en;
                    }
                }
            ...
            adapter.log.error('[ALEX3] Cannot process "' + id + '": ' + e);
        }
    }
    ...
    function controlPlay(id, state, callback) {
        adapter.getForeignObject(id + '.' + state, function (err, obj) {
            if (!obj || obj.type !== 'state') {
                if (err) adapter.log.error('[ALEX3] Cannot control non state: ' + id + ' ' + (obj ? obj.type : 'no object'));
                if (callback) callback();
                return;
            }


            adapter.log.debug('[ALEX3] Set "' + id + '.' + state + '" to true');

            adapter.setState('smart.lastObjectID', id + '.' + state, true);

            adapter.setForeignState(id + '.' + state, true, function (err) {
                if (err) adapter.log.error('[ALEX3] Cannot control device: ' + err);
                if (callback) callback();
            });
        });
    }
    ...
    function controlValue(channelId, stateId, value, delta, callback) {
        adapter.getForeignObject(channelId + '.' + stateId, function (err, obj) {
            if (!obj || obj.type !== 'state') {
                if (err) adapter.log.error('[ALEX3] Cannot control non state: ' + channelId + ' ' + (obj ? obj.type : 'no object'));
                if (callback) callback();
                return;
            }
            ...
            if (delta) {
                adapter.getForeignState(channelId + '.' + stateId, value, function (err, state) {
                    ...
                    adapter.log.debug('[ALEX3] Set "' + channelId + '.' + stateId + '" to ' + state.val);
                ...
                adapter.log.debug('[ALEX3] Set "' + channelId + '.' + stateId + '" to ' + value);
    ...
    this.process = function (request, isEnabled, callback) {
        if (!request || !request.directive || !request.directive.header) {
            adapter.log.error('[ALEX3] Invalid request: no header!');
            return;
        }
        ...
        switch (request.directive.header.name) {
            ...
            case 'SetMute':
                ...
                //   }
                // }
                adapter.log.debug('[ALEX3] ALEXA SetMute: ' + request.directive.endpoint.cookie.id);
            ...
            case 'SetVolume':
                ...
                //   }
                // }
                adapter.log.debug('[ALEX3] ALEXA SetVolume: ' + request.directive.endpoint.cookie.id);
            ...
            case 'AdjustVolume':
                ...
                //   }
                // }
                adapter.log.debug('[ALEX3] ALEXA AdjustVolume: ' + request.directive.endpoint.cookie.id);
            ...
            case 'Next':
                ...
                //   }
                // }
                adapter.log.debug('[ALEX3] ALEXA ' + request.directive.header.name + ': ' + request.directive.endpoint.cookie.id);
            ...
            default:
                adapter.log.warn('[ALEX3] got unknown command from alexa: ' + request.directive.header.name);
                break;
        }
    };
    ...
    this.getDevices = function (callback) {
        ...
        adapter.getObjectView('system', 'state', {}, function (err, _states) {
            adapter.getObjectView('system', 'channel', {}, function (err, _channels) {
                adapter.getObjectView('system', 'device', {}, function (err, _devices) {
                    let states = {};
                    ...
                    for (let k = result.length - 1; k >= 0; k--) {
                        if (result[k].disabled) {
                            ...
                            adapter.log.debug('[ALEX3] Created entertainment ALEXA device: ' + result[k].friendlyName);
                        }
                    }
                    states = null;
                    channels = null;
