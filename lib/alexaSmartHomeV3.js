function AlexaSH3(adapter) {
    ...
    function finalProcess(channels) {
        ...
        for (var channelId in channels) {
            if (channels.hasOwnProperty(channelId)) {
                for (var f = 0; f < channels[channelId].cookie.friendlyNames.length; f++) {
                    var obj = JSON.parse(JSON.stringify(channels[channelId]));
    ...
    function processState(states, id, channels) {
        try {
            ...
            var type = types[states[id].common.role];
            ...
            // Get channel ID
            var parts        = id.split('.');
            var stateId      = parts.pop();
            var channelId    = parts.join('.');
            if (!states[channelId]) {
                return;
            }
            var friendlyName = states[channelId].common.smartName;

            if (typeof friendlyName === 'object' && states[channelId].common.smartName) {
                friendlyName = states[channelId].common.smartName[lang] || states[channelId].common.smartName.en;
            }
            ...
            var friendlyDescription = (states[channelId].common.name || id);
            ...
            for (var n = 0; n < friendlyNames.length; n++) {
                if (!channels[channelId]) {
                    channels[channelId] = {
                        enabled:             states[channelId].common.smartName !== false,
                        endpointId:		     '',
                ...
                var found = false;
                for (var c = 0; c < channels[channelId].capabilities.length; c++) {
        ...
        adapter.getForeignObject(channelId + '.' + stateId, function (err, obj) {
            ...
            if (delta) {
                adapter.getForeignState(channelId + '.' + stateId, value, function (err, state) {
                    ...
                    adapter.setForeignState(id + '.' + stateId, state.val, function (err) {
                        if (err) adapter.log.error('Cannot control device: ' + err);
                        if (callback) callback();
                    });
    ...
    this.process = function (request, callback) {
        ...
        switch (request.directive.header.name) {
            case 'Discover':
                ...
                for (var j = smartDevicesCopy.length - 1; j >= 0; j--) {
            ...
            adapter.objects.getObjectView('system', 'channel', {}, function (err, _channels) {
                adapter.objects.getObjectView('system', 'device', {}, function (err, _devices) {
                    ...
                    if (_channels && _channels.rows) {
                        for (var c = 0; c < _channels.rows.length; c++) {
                    ...
                    if (_devices && _devices.rows) {
                        for (var d = 0; d < _devices.rows.length; d++) {
                    ...
                    var result = finalProcess(channels);
